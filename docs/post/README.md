...
|---|---|---|---|---|
| 2026-09-11 | 2026-09-25 | finished | likely[^conf] | 4 |

> **Abstract.** Medication normalization tools reliably map a raw drug string to its RxNorm *ingredient* (RxMap, [Korpela et al 2026](#references), reports F1 ≈ 0.97), but the code a pharmacy or interaction checker needs is the *clinical drug*: ingredient, strength, and dose form resolved together. I ask how far a small retrieval model gets at that level, using a labeled dataset that costs nothing to build: the VA National Drug File (VANDF) is a source vocabulary inside RxNorm, so every VA name that shares a concept identifier with an RxNorm clinical drug is, by NLM's own curation, the same drug. From the 2026-09-08 release I extract 14,372 (VA string, SCD/SBD) pairs over 8,315 targets and a 27,287-candidate pool, split *by ingredient* so the test set contains only drugs whose ingredients were never seen in training. Exact string match scores 0; TF-IDF character n-grams reach acc@1 0.509; a MiniLM bi-encoder fine-tuned with a contrastive loss and ingredient-matched hard negatives reaches 0.836; a 40-line rule that pre-computes RxNorm-style concentrations adds 5 points (0.886); and an 18-run grid shows domain pre-training (SapBERT) is worth a further +8.6 points of validation accuracy, more than either of the other factors; a second 18-run sweep re-checks the smallest factor on six ingredient draws, and ingredient-matched hard negatives beat in-batch-only on every one of them (+1.6 points test acc@1, 95% CI 0.8–2.4). The final model scores **acc@1 0.931 (95% CI 0.918–0.941), recall@5 0.984** on the published test split; re-drawing the ingredient split five more times and retraining puts the same recipe at **0.907 ± 0.019** (range 0.874–0.931), with the published draw the most favorable of the six and a training-seed standard deviation of 0.0045 (measured in [Part II](https://withinnoise.dev/blog/posts/vandf-rxnorm-interventions/) over twelve draw-and-seed pairs; the three seeds run here gave 0.001). Because a 7% error rate is not deployable without review, I calibrate a confidence score and choose thresholds on validation only: 79% of answerable strings can be auto-accepted at 98.8% precision, or 46% at 98.2% once real drugs with no clinical-drug concept are included. Remaining errors split evenly across strength, dose form, ingredient, and brand-versus-generic twins, and a share of the strength errors are underdetermined by the input string. Limitations: VA strings only; a headline that should be read as a band of about two points rather than a point estimate, because the ingredient draw moves it by two to three times the sampling error; and calibration thresholds fit on one split. Code, data, model, and every run are public.

## Summary without the terminology

A VA pharmacy records each medication under the VA's own name for it, such as `PAROXETINE HCL 40MG TAB`. Most other health systems, and most software that checks for drug interactions or selects patients for research, identify drugs by codes from RxNorm, a national vocabulary maintained by the National Library of Medicine, in which the same product is `paroxetine hydrochloride 40 MG Oral Tablet`. Exchanging records between the two requires matching each VA name to the correct RxNorm entry. Existing tools do this reliably for the active ingredient (paroxetine), but a pharmacy needs the complete product: ingredient, strength, and form (tablet, solution, patch) together. This report asks how accurately a small machine-learning model can make the complete match.

The training data cost nothing to assemble. The Library's staff have matched VA names to RxNorm entries by hand for two decades and publish the matches, which gives 14,372 VA names with a known correct answer. Part of them trains the model and the rest checks it. The check uses only drugs whose active ingredients the model never saw during training, so that it cannot succeed by recognizing a name it has already been shown.

1. **Comparing the text directly does not work.** Not one VA name is identical to its RxNorm name, even with capitals and punctuation ignored. A standard text-similarity method, which scores names by the fragments of letters they share, finds the correct ingredient 98% of the time but the correct complete product only 51% of the time, because the two vocabularies write strength and form differently: the VA's `TAB,SA` is RxNorm's `Extended Release Oral Tablet`.
2. **A model trained on the matched pairs identifies the complete product in most cases.** The model converts each name into a list of numbers and is trained so that names of the same product receive similar numbers. During training it is shown each correct pair together with a name of the same drug at a different strength or in a different form, so that it learns to separate close alternatives. The first version chose the correct product 84% of the time.
3. **A short arithmetic rule corrected errors the model had not learned to avoid.** Many of the first model's errors were unit conversions: the VA writes a concentration as a percentage, RxNorm as milligrams per millilitre. The model had learned an approximate association between the two notations rather than the arithmetic. A 40-line program that performs the conversion and adds the result to the VA name raised accuracy by 5 points, to 89%. Errors that follow a fixed procedure are better handled by writing the procedure than by more training.
4. **The choice of starting model mattered more than any other choice.** Three publicly available models were compared as starting points. SapBERT, which had previously been trained to recognize that different medical terms name the same concept, was 8.6 points more accurate than two general-purpose English models. The final model chooses the correct product 93.1% of the time, and the correct product is among its first five suggestions 98.4% of the time.
5. **The measured accuracy depends on which drugs are set aside for testing, by about two points.** The division of ingredients between training and testing was drawn five more times and the model retrained on each. Accuracy ranged from 87.4% to 93.1%, with an average of 90.7%; the first division, which produced the 93.1%, was the most favourable of the six. The accurate statement is about 91%, with an uncertainty of about two points in either direction.
6. **The model can indicate when it is likely to be wrong.** A model that is wrong in about one case in eleven cannot be used without review. Its similarity score was converted into an estimate of the probability that the answer is correct, and a cut-off was chosen on separate data. Names above the cut-off are accepted automatically; the rest go to a reviewer together with the first five suggestions. On names that have a correct answer, 79% were accepted, and 98.8% of those were correct. When the file also contains real drugs that have no complete-product entry in RxNorm, such as dose packs, for which every answer is wrong, 46% were accepted, at 98.2%.
7. **Some of the remaining errors cannot be corrected from the name alone.** The final model's errors are divided almost evenly among strength, form, ingredient, and confusion of a brand product with its generic. Some VA names lack information that the RxNorm entry contains: `MANNITOL 250MG/ML INJ` corresponds to a 50 mL product, and the volume does not appear in the name. For such names the correct response is to refer them to a person, and the confidence estimate mostly does so.

The model is suitable as a suggestion tool for a person performing the mapping, since the correct answer is among its first five suggestions about 98% of the time. Whether to accept half to three-quarters of a file automatically, with about 2% of the accepted answers wrong, is a policy decision that depends on what uses the mapping. It is not suitable for fully automatic use.
...
The effects are roughly additive, and the best cell is best on every axis, so there is no interesting interaction to report. On H5: the encoder dominates, and the direction is instructive. `bge-small` is newer and scores higher than MiniLM on general retrieval benchmarks, yet the two are indistinguishable here; SapBERT, which is older and not a general-purpose embedder at all, wins by a wide margin because its pre-training objective (UMLS synonym alignment) is almost this task. Two cautions. Each cell is one seed on one split. The training seed moves test acc@1 with a standard deviation of about 0.0045 ([Part II](https://withinnoise.dev/blog/posts/vandf-rxnorm-interventions/) §5.1; the three seeds of §5.8 gave 0.001, and the ±0.5 point in §5.2 was the GPU and library stack), and the split moves it by about 0.02 between draws; the encoder and normalizer effects lie well outside both, and the +3.0 negatives effect does not, which is why §5.9 re-runs it on every split. And the normalizer's contribution shrinks with SapBERT (+4.0 on its best cell vs +5.6 for MiniLM), consistent with a stronger encoder having absorbed some of the conversions.
...
- **The training seed matters less than the split, though by less than these three runs suggest.** Seeds 1, 2, and 42 on the published split give 0.930, 0.931, and 0.932 on test and 0.880–0.881 on validation. A standard deviation estimated from three runs is uncertain by a factor of about 0.5 to 6; [Part II](https://withinnoise.dev/blog/posts/vandf-rxnorm-interventions/) (§5.1) trained two seeds on each of the six draws on the A100 and measures it at 0.0045, about a quarter of the split's standard deviation, which is enough to move a single run by up to a point. The half-point difference between the local and Colab replications in §5.2 used the same seed, and was the hardware and library stack.
...
- **Six splits, not sixty.** The final recipe (§5.8) and the hard-negatives factor (§5.9) were run on six ingredient draws; the baselines, the encoder and normalizer factors, and the calibration were run on one. Six draws pin the split standard deviation to a factor of about two, so "0.907 ± 0.019" is a band, not a measurement of the band's width. The two factors not re-run have single-split effects of +8.6 and +4.8, several times that band. Training-seed noise is smaller than the split's but not negligible: a standard deviation of 0.0045 in [Part II](https://withinnoise.dev/blog/posts/vandf-rxnorm-interventions/), against 0.001 from the three seeds of §5.8.
