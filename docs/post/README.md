...
Sweep `uukeyzw7` ([W&B](https://wandb.ai/kettle-labs/rxnorm-vandf/sweeps/uukeyzw7)): SapBERT with the normalizer on, the three hard-negative strategies, on each of the six splits, 18 runs on one Colab A100 in about 35 minutes. The three v1 cells reproduced the original sweep's v1 cells to four decimals (0.9275 / 0.9232 / 0.9096 test), which says two things: the pipeline is unchanged, and Colab's stack reproduces a run at the same seed on the same GPU type (to four decimals here, and to within 0.0005 in three later repeats reported in [Part II](https://kettlelabs.dev/blog/posts/vandf-rxnorm-interventions/), Appendix A), so the half-point difference in §5.2 was the 1070 against the A100.
