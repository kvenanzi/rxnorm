...
Everything runs from the commands in the repository README under "The follow-up". Sweeps `nypttmi8` (main) and `ad9nakej` (the aux-off cells on v3 to v6 re-run after the loader fix noted in Appendix A; `j1zdu28j` was registered with the pilot's 0.2 weight and never run), `2exoct4h` (steps control), `ocmpf5na` (size control); W&B group `kfold-k7`; artifacts `vandf-rxnorm-multi:v0` and `vandf-rxnorm-kfold:v0` for the data, `vandf-rxnorm-predictions` per fold, `vandf-rxnorm-biencoder-final-all` for the all-data model. The published `vandf-rxnorm-pairs`, `vandf-rxnorm-splits`, and `vandf-rxnorm-biencoder` artifacts are untouched.
