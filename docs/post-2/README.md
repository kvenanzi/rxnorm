...
|---|---|---|---|---|
| 2026-09-14 | 2026-09-25 | finished | likely[^conf] | 3 |

> **Abstract.** The [first report](https://withinnoise.dev/blog/posts/vandf-rxnorm-biencoder/) (hereafter Part I) fine-tuned a small bi-encoder to map VA National Drug File (VANDF) names to RxNorm clinical drugs and, by re-drawing the ingredient-level split five additional times, estimated its accuracy on unseen ingredients at 0.907 ± 0.019, against 0.931 on the published draw. Its concluding section listed interventions expected to change that figure. This report evaluates three of them under the same design: every arm is trained on each of the same six ingredient draws and two training seeds, and an effect is credited only if it exceeds the split-to-split spread of about two points rather than the value on a single favourable draw. The interventions are (1) a second source vocabulary from the same RxNorm release, the FDA Structured Product Label names (MTHSPL), which is the only large source at UMLS restriction level 0 other than the two already used, and which contributes 48,985 labelled pairs whose surface form differs substantially from the VA's; (2) an auxiliary classification head, trained jointly and discarded at inference, that predicts the target's strength from the query embedding ([Caruana 1997](#references)), directed at the error category shared by every earlier model; and (3) seven-fold cross-validation by ingredient ([Stone 1974](#references)), which yields a single out-of-fold estimate over every held-out string, a final model trained on every ingredient family, and a test of whether abstention thresholds chosen on one set of folds transfer to another. A pilot on the published split found that the head's loss is approximately fifty times the contrastive loss at initialization and that every weight above 0.01 lowers validation accuracy monotonically; the factorial grid therefore runs the head at 0.01. Neither training intervention changes accuracy on VA strings. Over twelve paired (draw, seed) units, the SPL strings change VA test acc@1 by −0.0003 (95% CI −0.006 to +0.005; 6 of 12 units positive) and the head by −0.0013 (−0.003 to 0.000; 1 of 12), and the head leaves unchanged the strength accuracy it was designed to raise. The SPL strings do raise accuracy on held-out SPL strings, from 0.709 to 0.833 (+0.123; 12 of 12), and a control capped at 9,300 SPL rows retains all of that increase on the two draws on which it was run. Seven-fold cross-validation of the VA-only recipe gives a pooled out-of-fold acc@1 of 0.894 (Wilson 95% 0.889–0.899, n = 12,227), within the six-draw band, and a 99%-precision threshold chosen on six folds achieves between 0.986 and 0.998 on the seventh. The grid also revises one estimate of Part I: on the A100 the training seed contributes a standard deviation of 0.0045 in test acc@1, not 0.001, so the split contributes about four times the seed's standard deviation rather than twenty.

## Summary without the terminology

Part I trained a small model that matches VA drug names, such as `PAROXETINE HCL 40MG TAB`, to the corresponding complete-product entries of RxNorm, the national drug vocabulary, such as `paroxetine hydrochloride 40 MG Oral Tablet`. On drugs whose ingredients it had not seen in training, it chose the correct product about 91% of the time. Part I ended with a list of changes that might improve it; this report tests three. Part I also found that accuracy changes by about two points depending only on which drugs are set aside for testing, so a change tested on one such division could appear to help or harm by chance. Every change here was therefore tested on the same six divisions, trained twice on each with a different random seed (the setting that determines the order of the training examples and other random choices), and credited only if it changed accuracy consistently across the twelve comparisons.

1. **Names from a second source did not improve the matching of VA names, but did improve the matching of names from that source.** The FDA's drug-label names are also linked to RxNorm by the National Library of Medicine, and are written in a different style: `PAROXETINE HYDROCHLORIDE 40 mg ORAL TABLET, FILM COATED [Paroxetine]` rather than `PAROXETINE HCL 40MG TAB`. Adding about 30,000 of them to the 9,300 VA training names left accuracy on VA names unchanged: the average change was −0.03 points, and it was positive in 6 of the 12 comparisons. Accuracy on FDA names rose from 71% to 83%, in all 12. Two further runs showed that the increase comes from the FDA names themselves and not from the longer training that more data requires: training four times as long on VA names alone lowered accuracy on FDA names, and a third of the FDA names produced the whole increase on the two divisions tested.
2. **Training the model to predict strength did not improve its strength accuracy.** Wrong strength was the most common error of every model in Part I. A second task was added during training, to predict the correct product's strength from the VA name, and removed afterwards; the expectation was that the model would learn to represent strength more exactly. Strength accuracy did not change, and overall accuracy fell by 0.1 to 0.2 points; it was higher with the added task in at most one of the 12 comparisons. A preliminary run found that giving the added task more weight lowered accuracy further.
3. **A more complete estimate of accuracy agrees with Part I.** Part I's model was trained on 70% of ingredients and checked on the rest. Here the ingredients were divided into seven groups, and seven models were each trained with one group set aside, so that every VA name was tested once by a model that had not seen its ingredients. Accuracy over the 12,227 names tested this way is 89.4%, within Part I's range, and the test set Part I published was again the most favourable group (93.2%). A final model was then trained on every ingredient, which is the version intended for use. Its expected accuracy on new drugs is the 89.4% from the seven groups, not the 93.1% reported in Part I.
4. **The cut-off for automatic acceptance holds when it is chosen on enough data.** In Part I, a cut-off intended to accept only answers that are 99% correct achieved 98.2%. Here, cut-offs chosen on six of the groups and applied to the seventh achieved between 98.6% and 99.8%, 99.0% on average, while accepting about half of the names. The shortfall in Part I is therefore explained by the small number of names on which its cut-off was chosen, not by a property of the task. A cut-off intended for 95% was less consistent, achieving between 92.2% and 96.7%.
5. **The random seed affects accuracy more than Part I estimated.** Two runs of each setting on each division show that the seed alone moves accuracy with a standard deviation of 0.45 points, not the 0.1 points that Part I estimated from three runs. The choice of drugs set aside for testing still has about four times that effect.

Neither change to training improved the matching of VA names. The experiment was precise enough to detect a change of 0.2 points, which is the decrease the strength task produced, so the absence of an improvement indicates that neither change has an effect of the size expected, not that the measurement was too coarse to find one. One possible explanation, which neither experiment tests directly, is that part of the remaining error cannot be removed because the VA name does not contain the information required (Part I, §5.7).
...
- **Code:** the same repository as Part I; the commands are listed in its README under "The follow-up". Nothing Part I depends on has changed: its data folder, scripts, figures, and numbers are reproduced by the same commands, and the default dataset build emits byte-identical SQL. Every new behaviour is controlled by a new flag or a new configuration field with a default.
